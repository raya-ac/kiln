...
# Package every SwiftPM resource bundle, including SwiftTerm's Metal shaders.
# Deep bundle layout keeps codesign valid and gives runtime loaders a stable
# Contents/Resources/<package>_<target>.bundle location.
for SPM_RES_BUNDLE in "$BIN_PATH/"*.bundle; do
  [ -d "$SPM_RES_BUNDLE" ] || continue
  RESOURCE_NAME="$(basename "$SPM_RES_BUNDLE" .bundle)"
  DEST_BUNDLE="$APP/Contents/Resources/$RESOURCE_NAME.bundle"
...
<plist version="1.0">
<dict>
  <key>CFBundleIdentifier</key><string>$BUNDLE_ID.resources.$RESOURCE_NAME</string>
  <key>CFBundleInfoDictionaryVersion</key><string>6.0</string>
  <key>CFBundleName</key><string>$RESOURCE_NAME</string>
